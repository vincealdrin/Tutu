...
def scrape(sources_json_path):
    awis_count = 0
    count = 0
    folder_path = './business-articles-' + str(uuid4())
        ...
        for news_source in news_sources:
            for url in news_source['urls']:
                ...
                for article in source.articles:
                    ...
                    if not 'business' in article.url or not 'money' in article.url or not 'economy' in article.url:
                        print('\n(NOT BUSINESS) Skipped: ' + str(article.url) + '\n')
                        should_slp = False
                        continue
