...
def scrape(sources_json_path):
    awis_count = 0
    count = 0
    folder_path = './lifestyle-articles-' + str(uuid4())
        ...
        for news_source in news_sources:
            for url in news_source['urls']:
                ...
                for article in source.articles:
                    ...
                    if not 'lifestyle' in article.url or not 'life' in article.url or not 'life-and-style' in article.url:
                        print('\n(NOT LIFESTYLE) Skipped: ' + str(article.url) + '\n')
                        should_slp = False
                        continue
